...
def test_create_two_versions_and_list():
    ...
    try:
        # Version 1
        resp1 = client.post(f"/api/v1/projects/{project.id}/rythmo/versions", json={"comment": "Version initiale"})
        ...
        r1_id = replicas[0].id
        resp_patch = client.patch(f"/api/v1/replicas/{r1_id}", json={"text": "Bonjour modifié"})
        assert resp_patch.status_code == 200

        # Version 2 après modification
        resp2 = client.post(f"/api/v1/projects/{project.id}/rythmo/versions", json={"comment": "Après modif texte"})
        ...
        # Lister les versions
        resp_list = client.get(f"/api/v1/projects/{project.id}/rythmo/versions")
...
def test_consult_version_and_restore():
    ...
    try:
        ...
        resp1 = client.post(f"/api/v1/projects/{project.id}/rythmo/versions", json={"comment": "v1"})
        v1_id = resp1.json()["id"]
        v1_number = resp1.json()["version_number"]

        # Modifier
        r1_id = replicas[0].id
        client.patch(f"/api/v1/replicas/{r1_id}", json={"text": "Texte modifié v2", "typo_codes": {"italique": True}})

        # Version 2
        resp2 = client.post(f"/api/v1/projects/{project.id}/rythmo/versions", json={"comment": "v2"})
        v2_id = resp2.json()["id"]

        # Consulter version 1
        resp_get_v1 = client.get(f"/api/v1/projects/{project.id}/rythmo/versions/{v1_id}")
        ...
        # Consulter version 2
        resp_get_v2 = client.get(f"/api/v1/projects/{project.id}/rythmo/versions/{v2_id}")
        assert resp_get_v2.status_code == 200
        snap_v2 = resp_get_v2.json()["snapshot"]
        assert any(r["text"] == "Texte modifié v2" for r in snap_v2)

        # Restaurer version 1
        resp_restore = client.post(f"/api/v1/projects/{project.id}/rythmo/versions/{v1_id}/restore")
        ...
        resp_replicas = client.get(f"/api/v1/projects/{project.id}/replicas")
...
def test_compare_versions():
    ...
    try:
        ...
        r1_id = replicas[0].id
        client.patch(f"/api/v1/replicas/{r1_id}", json={"text": "Bonjour modifié"})
        ...
        # Comparer
        resp_cmp = client.get(f"/api/v1/projects/{project.id}/rythmo/versions/compare?from={v1_id}&to={v2_id}")
...
def test_compare_by_version_numbers():
    studio, project, media, replicas = _setup_project_with_replicas()
    try:
        client.post(f"/api/v1/projects/{project.id}/rythmo/versions", json={})
        r1_id = replicas[0].id
        client.patch(f"/api/v1/replicas/{r1_id}", json={"text": "changed"})
        client.post(f"/api/v1/projects/{project.id}/rythmo/versions", json={})

        resp = client.get(f"/api/v1/projects/{project.id}/rythmo/versions/compare?from_version=1&to_version=2")
...
def test_restore_creates_correct_state_with_typo_codes():
    ...
    try:
        # V1 sans typo
        v1 = client.post(f"/api/v1/projects/{project.id}/rythmo/versions", json={}).json()
        # Modifier typo
        client.patch(f"/api/v1/replicas/{replicas[0].id}", json={"typo_codes": {"crochets": True}})
        v2 = client.post(f"/api/v1/projects/{project.id}/rythmo/versions", json={}).json()
        assert any(r.get("typo_codes", {}).get("crochets") for r in v2["snapshot"])

        # Restore v1 (sans typo)
        resp = client.post(f"/api/v1/projects/{project.id}/rythmo/versions/{v1['id']}/restore")
        assert resp.status_code == 200
        db = TestingSessionLocal()
        media_ids = [m.id for m in db.query(MediaAsset).filter(MediaAsset.project_id == project.id).all()]
