...
from app.models import MediaAsset, Studio, Project, User
...
def test_transcription_fr_small():
    ...
    try:
        ...
        assert result.get("artifact_id"), "Un artifact doit être produit"
        assert result.get("result_path"), "Le résultat doit être persisté dans le stockage"

        # §5.4 — Séparation calcul/stockage : la transcription persiste un
        # artifact dans le stockage objet (S3), pas de lignes en DB applicative.
        # On vérifie la lisibilité de l'artifact via l'internal API.
        api = get_worker_api()
        artifact = api.get_artifact(uuid.UUID(result["artifact_id"]))
        assert artifact is not None, "Artifact de transcription introuvable"
